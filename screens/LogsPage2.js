...
export default function Logspage2() {
  ...
  useEffect(() => {
    const fetchDataAndLogs = async () => {
      try {
        const storedLogs = await getItem('sensorLogs2');
        if (storedLogs) {
  ...
  const deleteLogsData = async () => {
    try {
      await setItem('sensorLogs2', JSON.stringify([]));
      setLogs([]);
  ...
  const appendSensorDataToLogs = async () => {
    try {
      ...
      const newLogData = {
        temperature: tempt2, // Replace with the actual data
        humidity: hum2, // Replace with the actual data
        turbidity: turb2, // Replace with the actual data
        phLevel: ph2, // Replace with the actual data
        ds18b20temp: ds2, // Replace with the actual data
        lux: light2, // Replace with the actual data
      ...
      await setItem('sensorLogs2', JSON.stringify(updatedLogsWithNewLog));
